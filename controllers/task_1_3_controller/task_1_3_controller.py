...
# prox_sensors = []
# for i in [0,1,2,5,6,7]:  # TODO: only front 4 sensors should be enough?
...
#     pos[2] += dright * diam / l
#     pos[2] = pos[2] % math.pi
#     print(pos[0], pos[1], pos[2]+(math.pi/2))
#     return dright
#
#
# def move_straight():
...
# print('initial', pos[0], pos[1], pos[2]+(math.pi/2))
# robot.step(timestep)
# [move_straight() for _ in range(1000)]
...
def move_straight():
    ...
    pos[0] -= drobot * math.cos(pos[2])
    pos[1] += drobot * math.sin(pos[2])
    # path.step(encoder_left, encoder_right)
    print(pos[0], pos[1], pos[2]+(math.pi/2))
    trace.append(deepcopy(pos))
    return drobot
...
while counter < 5:
    ...
    while math.hypot(pos[0] - target[0], target[1] - pos[1]) > dist_eps and len(trace) < T:
        ...
        motor_left.setVelocity(1.0)
        motor_right.setVelocity(1.0)
        [move_straight() for _ in range(20)]
        #move_straight()
    print(pos)
